module RedmineIssueStatusChange
  class CustomFiledChange < Redmine::Hook::ViewListener
    ...
    def update_field(context)
      ...
      if @issue.status.name=="Re-opened"
         resolved_cfss = CustomField.find_by_name('Resolved By')
         resolution_cfs = CustomField.find_by_name('Resolution Type')
         resolved_cvss = CustomValue.where(:customized_type => "Issue", :customized_id => @issue.id, :custom_field_id => resolved_cfss.id).last
         resolution_cf= CustomValue.where(:customized_type => "Issue", :customized_id => @issue.id, :custom_field_id => resolution_cfs.id).last
         if resolved_cvss.present?
            resolved_cvss.update_attribute :value,""
            if resolution_cf.present?
               resolution_cf.update_attribute :value,""
            end
         end
      end
    end
